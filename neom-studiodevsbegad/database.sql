-- ============================================================
--  Neom Studio — سكربت قاعدة البيانات الكامل (Supabase / PostgreSQL)
--  ده السكربت الوحيد اللي محتاجه — بيعمل كل حاجة مرة واحدة:
--    • كل الجداول + عمود آيدي ديسكورد
--    • قاعدة "Yaro = مالك"
--    • التخزين (رفع الصور والملفات)
--    • الصلاحيات + تعطيل RLS (بيحل خطأ: violates row-level security)
--
--  طريقة التشغيل:
--    Supabase ← SQL Editor ← New query ← الصق الملف كله ← RUN
--  آمن تمامًا إنك تشغّله حتى لو المشروع موجود قبل كده (مش هيمسح أي بيانات).
-- ============================================================
...
create table if not exists profiles (
  ...
  age          text,
  discord      text,
  discord_id   text,               -- آيدي ديسكورد الرقمي (للمنشن)
...
-- لو الجدول موجود قبل كده، نضيف العمود الجديد (آمن لو موجود بالفعل)
...
-- (متغلّفة عشان لو فيه أي مشكلة صلاحيات، ما توقفش باقي السكربت)
do $$
begin
...
exception when others then
  raise notice 'تم تخطي إعداد سياسات التخزين (صلاحيات): %', sqlerrm;
end $$;
...
-- ده أهم جزء لحل خطأ: new row violates row-level security policy
do $$
declare t text;
begin
  foreach t in array array['profiles','warnings','tasks','task_tiers','task_files','submissions','submission_files']
  loop

...
// ============================================================
import { state, updateUser, addWarning, clearWarnings, createTask, loadData } from "./store.js";
import { tierDuration } from "./discord.js";
...
export function adminHTML(){
  ...
  <div class="fade">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;flex-wrap:wrap">
      ...
      <button class="btn btn-ghost btn-sm" data-refresh-admin title="تحديث البيانات">${icon("refresh-cw",14)} تحديث</button>
    ...
    <div class="bdt" style="padding:16px;background:#0e1119">
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:14px">
        ...
        <div class="card2 bdsoft" style="border-radius:9px;padding:9px 11px;border-color:rgba(232,127,135,.3)">
          ...
          ${u.rank === "مالك"
            ? `<div class="mono" style="font-size:12.5px;font-weight:600;color:var(--violet)" dir="rtl">${icon("shield",11)} محمية (مالك)</div>`
      ...
      <div style="margin-bottom:12px">
        <div class="lbl mono" style="font-size:10px">${icon("message-square",11)} آيدي ديسكورد (للمنشن) ${u.discord_id?`<span class="badge b-teal" style="margin-inline-start:6px">${icon("check",11)} متوفر</span>`:`<span class="badge b-amber" style="margin-inline-start:6px">${icon("triangle-alert",11)} فاضي</span>`}</div>
        <div style="display:flex;gap:6px">
          <input class="field num" dir="ltr" inputmode="numeric" data-discordid-input="${esc(u.id)}" value="${esc(u.discord_id||"")}" placeholder="123456789012345678" style="flex:1"/>
          <button class="btn btn-ghost btn-sm" data-setdiscordid="${esc(u.id)}">${icon("check",14)} حفظ</button>
        </div>
      </div>
...
export function bindAdmin(root){
  // زر التحديث اليدوي
  root.querySelector("[data-refresh-admin]")?.addEventListener("click", async (e) => {
    const btn = e.currentTarget; const old = btn.innerHTML;
    btn.disabled = true; btn.innerHTML = `${icon("loader-circle",14,"spin")} جارٍ التحديث…`; drawIcons();
    try{ await loadData(); toast("تم تحديث البيانات","ok"); rerender(); }
    catch(err){ btn.disabled=false; btn.innerHTML=old; drawIcons(); toast(err.message,"err"); }
  });

  // تبديل التبويب — بيعمل تحديث للبيانات كمان
  root.querySelectorAll("[data-atab]").forEach(b => b.addEventListener("click", async () => {
...
function bindUsers(root){
  ...
  root.querySelectorAll("[data-setdiscordid]").forEach(btn => btn.addEventListener("click", async () => {
    const id = btn.dataset.setdiscordid;
    const input = root.querySelector(`[data-discordid-input="${id}"]`);
    const val = (input?.value || "").trim();
    if (val && !/^\d{5,25}$/.test(val)) return toast("آيدي ديسكورد لازم يكون أرقام بس","err");
    try{ await updateUser(id, { discord_id: val || null }); toast("تم حفظ آيدي ديسكورد","ok"); rerender(); }catch(e){ toast(e.message,"err"); }
  }));
...
async function doCreateTask(root, btn){
  ...
  root.querySelectorAll(".nt-tier").forEach(row => {
    ...
    tiers.push({
      ...
      deadline: new Date(Date.now() + days*DAY + hours*HOUR).toISOString(),
      durationText: tierDuration(days, hours),
    });
  });

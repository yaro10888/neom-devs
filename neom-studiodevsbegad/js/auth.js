...
// ============================================================
import { state, signup, login, loadData, refreshMe, isAdmin, saveSession, clearSession } from "./store.js";
...
async function doLogin(btn){
  ...
  try{
    ...
    state.session = u;
    saveSession(u.id);   // تسجيل الدخول يفضل محفوظ حتى بعد قفل الموقع
...
async function doSignup(btn){
  const f = {
    ...
    discord: val("su-discord"),
    whatsapp: val("su-whatsapp"), roblox: val("su-roblox"),
  ...
  try{
    ...
    state.session = u;
    saveSession(u.id);   // يفضل مسجّل دخول (حتى لو الحساب لسه تحت التوثيق)
